...
~> `-azure-blob-service-principal-id`, `-azure-blob-service-principal-secret`, and `-azure-blob-tenant-id` are introduced in v1.20.1+ent.

- `-azure-blob-service-principal-id` and `-azure-blob-service-principal-secret` - You must provide Service Principal (SPN) ID + Secret pair together.

- `-azure-blob-tenant-id` - Required field if using Service Principal (SPN) ID + Secret. Not required if you use Account Key only.
